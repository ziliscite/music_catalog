...
func TestHandler_Upsert(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mockFn: func() {
				mockService.EXPECT().Upsert(uint(1), &usertrackModel.LikeRequest{
					TrackID: "0Ot6e3wYVQQ1Us9PM977jE",
					IsLiked: &isLikedTrue,
				}).Return(true, nil)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			endpoint := `/track/upsert`

			payload := usertrackModel.LikeRequest{
				TrackID: "0Ot6e3wYVQQ1Us9PM977jE",
				IsLiked: &isLikedTrue,
			}
			...
			// aborting test due to missing call(s) -- biasanya karna salah http method
